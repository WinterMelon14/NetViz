...
class UserTraceRuntimeTests(unittest.TestCase):
    ...
    def project_context(self, project_root: Path, source_path: Path, *, local_modules=None, resources=None):
        ...
        return {
            ...
            "entry_relative_path": source_path.relative_to(project_root).as_posix(),
            "local_modules": [descriptor(path) for path in (local_modules or [])],
            "resources": [descriptor(path) for path in (resources or [])],
        }
    ...
    def test_integer_tensor_input_is_bounded(self):
        input_spec = {
            "kind": "tensor",
            ...
            "integer_max_exclusive": 5,
        }
        tensor = build_tensor_inputs([input_spec])[0]
        self.assertEqual(tensor.dtype, torch.int64)
        self.assertGreaterEqual(int(tensor.min()), 0)
        self.assertLess(int(tensor.max()), 5)

    def test_self_contained_file_produces_inline_trace(self):
        with patch("util.summary.fx.symbolic_trace", wraps=torch.fx.symbolic_trace) as symbolic_trace:
        ...
        self.assertEqual(symbolic_trace.call_count, 1)
        self.assertNotIn(str(self.source_path), json.dumps(result["trace"]["payload"]))
    ...
    def test_project_context_supports_sibling_imports_and_resource_identity(self):
        layers_path = self.root / "layers.py"
        resource_path = self.root / "weights.pt"
        layers_path.write_text("import torch\nclass Block(torch.nn.Module):\n    def forward(self, x): return x + 1\n", encoding="utf-8")
        resource_path.write_bytes(b"checkpoint")
        self.source_path.write_text(
            "import torch\nfrom layers import Block\n"
            "class UserModel(torch.nn.Module):\n"
            "    def __init__(self):\n        super().__init__()\n        self.block = Block()\n        self.weights = open('weights.pt', 'rb').read()\n"
            "    def forward(self, x): return self.block(x)\n",
            encoding="utf-8",
        )
        request = self.worker_request("project-sibling")
        request["source"]["content_sha256"] = hashlib.sha256(self.source_path.read_bytes()).hexdigest()
        request["project_context"] = self.project_context(self.root, self.source_path, local_modules=[layers_path], resources=[resource_path])

        result = run_trace(str(self.write_request(request)))

        self.assertEqual(result["type"], "success", result)

        layers_path.write_text(layers_path.read_text(encoding="utf-8") + "\nVALUE = 1\n", encoding="utf-8")
        changed = run_trace(str(self.write_request({**request, "run_id": "project-module-changed"})))
        self.assertEqual(changed["error"]["code"], "project_module_changed")

    def test_project_context_supports_package_relative_imports(self):
        ...
        request = self.worker_request("project-relative")
        request["source"] = {
            "file_path": str(model_path),
            "class_name": "UserModel",
            "content_sha256": hashlib.sha256(model_path.read_bytes()).hexdigest(),
        }
        request["project_context"] = self.project_context(self.root, model_path, local_modules=[layer_path])
    ...
    def test_selected_handle_bridge_runs_trace(self):
        selected_files = SourceHandles(picker=lambda: str(self.source_path))
        ...
        result = api.runUserTrace(request)

        self.assertEqual(result["type"], "success")
        self.assertEqual(result["trace"]["payload"]["model_name"], "UserModel")

    def test_inline_handle_bridge_runs_trace_and_remains_available_for_retry(self):
        handles = SourceHandles(temp_root=self.root / "inline-handles")
    ...
    def test_model_provided_example_tensors_are_validated_and_traced(self):
        ...
        request = self.worker_request("provider-int64")
        request["inputs"] = []
        request["input_provider"] = {"function_name": "netviz_example_inputs", "parameter_names": ["x"]}
        result = run_trace(str(self.write_request(request)))
        self.assertEqual(result["type"], "success")
